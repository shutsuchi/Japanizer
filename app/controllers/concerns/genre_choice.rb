module GenreChoice
  ...
  def age_post(range, pg1)
    case range
    when '1'
      @posts_pg = type_page_8(Post.joins(:user).age_20, pg1)
    when '2'
      @posts_pg = type_page_8(Post.joins(:user).age_30, pg1)
    when '3'
      @posts_pg = type_page_8(Post.joins(:user).age_40, pg1)
    when '4'
      @posts_pg = type_page_8(Post.joins(:user).age_50, pg1)
    when '5'
      @posts_pg = type_page_8(Post.joins(:user).age_60, pg1)
    when '6'
      @posts_pg = type_page_8(Post.joins(:user).age_70, pg1)
    when '7'
      @posts_pg = type_page_8(Post.joins(:user).age_80, pg1)
    else
